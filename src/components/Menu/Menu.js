			...
			<WrapperCard cardTitle="Resturant's Menu">
				{this.props.isResturant ? (
					<div>
							...
							<div className={classes.ControllBanel}>
								{!this.state.editMenu && (
									<button
										...
										onClick={this.openEditMenuHandler}
									>
										Add New
									</button>
								)}
						...
						{this.props.menuLoading && <Placeholder />}
						{menuContent}
					</div>
				) : (
					<div>{menuContent}</div>
...
const mapDispatchToProps = (dispatch) => {
	return {
		onToggleItem: (item, Authenticated, token, startLoading, stopLoading) =>
			dispatch(
				toggleItemAsync(item, Authenticated, token, startLoading, stopLoading)
			),
	};
};
